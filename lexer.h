...
// 表示从扫描器返回的标记或文本字符串
struct Item {
    ItemType type;    // 此项目的类型
    Pos pos;          // 此项目在输入字符串中的起始位置（以字节为单位）
    std::string val;  // 此项目的值
    int line;         // 此项目开始时的行号

    Item() : type(ItemType::ItemEOF), pos(0), val("EOF"), line(1) {}
...
// 词法分析器选项
struct LexOptions {
    bool emitComment = false;  // 发出itemComment标记
    bool breakOK = false;      // 允许break关键字
    bool continueOK = false;   // 允许continue关键字
};
...
private:
    ...
    std::string leftDelim_;     // 操作标记的开始
    std::string rightDelim_;    // 操作标记的结束
    Pos pos_;                   // 输入中的当前位置
    Pos start_;                 // 此项目的开始位置
    bool atEOF_;                // 我们已经到达输入结束并返回了eof
    int parenDepth_;            // () 表达式的嵌套深度
    int line_;                  // 1+看到的换行符数量
    int startLine_;             // 此项目的开始行
    bool insideAction_;         // 我们是否在操作内部？
    LexOptions options_;        // 词法分析器选项
    
    Pos lastPos_;               // 上一个项目的位置（用于调试）
    ItemType lastType_;         // 上一个项目的类型（用于调试）
    
    std::unordered_map<std::string, ItemType> keyMap_;  // 关键字映射
